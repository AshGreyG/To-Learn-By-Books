...
#theorem(number: "")[
  Draw the tree illustrating the process generated by $sans("count-change")$ procedure in making change for 11 cents. What are the orders of growth of the space and number of steps used by this process as the amount to be changed increases?
]

#proof[
  Consider to use a sequence $a_1, a_2, dots.c, a_k$ (assume in ascending order) to denote $k$ kinds of changes which have different denominations, and use $N$ to denote the amount.

  Since this is a recursive process, the orders of growth of the space is proportional to the depth of the recursive tree. And that's (because $min(a_1,a_2,dots.c,a_k)=a_1$ can be seen as a constant):

  $ Theta (N / a_1)=Theta (N) $

  We focus on counting the call of $sans("counter-change-iter")$, and we use $S(n,m)$ to denote the number of different ways of counting changes for amount $n$ using $m$ types of coin.

  - $m=0$, they are the leaves of this recursive tree and have no children;
  - $m=1$, there are $floor(n\/a_1)$ nodes from ($sans("count-change-iter n 1")$) to ($sans("count-change-iter 1 1")$), which will recurse after using the last type of coin. And there are $floor(n\/a_1)$ nodes from ($sans("count-change-iter n 0")$) to ($sans("count-change-iter 1 0")$). And finally a node ($sans("count-change-iter 0 1")$), if $n$ can be divided by $a_1$, so it's

  $ S(n,1)=2 floor(n/a_1)+1 $
  - $m=2$, similarly we can analyze that there is at most $floor(n\/a_2)+1$ times you can subtract $a_2$ cents before reaching zero or a negative value. We have

  $ 
    S(n,2)&=2 floor(n/a_2)+1+sum_(i=0)^(floor(n\/a_1)) S(n-a_2i,1) \
    &=2 floor(n/a_2)+1+sum_(i=0)^(floor(n\/a_1)) (2 floor((n-a_2i)/a_1)+1) \
    &=2 floor(n/a_2)+1+floor((2 n^2)/a_1^2+(2n)/a_1^2+(2n)/a_1+1)-sum_(i=0)^(floor(n\/a_1))floor((-2 a_2 i)/a_1) \
    &approx Theta(n^2)
  $
